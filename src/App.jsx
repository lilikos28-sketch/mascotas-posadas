...
const telLocal = (s)=>{ let d=digits(s); if(d.startsWith("549"))d=d.slice(3); else if(d.startsWith("54"))d=d.slice(2); if(d.startsWith("0"))d=d.slice(1); return d; };
const telValido = (s)=>{ const t=(s||"").toString().trim(); return /^[\d\s()+\-.]+$/.test(t) && telLocal(t).length===10; };
const waNum = (s)=>{ const d=telLocal(s); return d.length===10 ? "549"+d : digits(s); };
    ...
    <div style={{background:C.bg,color:C.ink,fontFamily:"'Plus Jakarta Sans', system-ui, -apple-system, Segoe UI, Roboto, sans-serif"}} className="min-h-screen w-full">
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
        *{box-sizing:border-box}.mp-scroll::-webkit-scrollbar{height:6px}.leaflet-container{font:inherit}
        ...
        @keyframes latido{0%,100%{transform:scale(1)}50%{transform:scale(1.025)}}
        @keyframes huella{0%{opacity:0;transform:scale(.5)}15%{opacity:1;transform:scale(1)}45%{opacity:1;transform:scale(1)}65%{opacity:0;transform:scale(1)}100%{opacity:0}}
        .mp-huella{animation:huella 2.2s ease-in-out infinite;display:inline-block}
        .mp-huella:nth-child(1){animation-delay:0s}
        .mp-huella:nth-child(2){animation-delay:.28s}
        .mp-huella:nth-child(3){animation-delay:.56s}
        ...
        .mp-brillo{background:linear-gradient(110deg,transparent 30%,rgba(255,255,255,.25) 50%,transparent 70%);background-size:200% 100%;animation:brillo 3s linear infinite}
    ...
    <div className="px-4">
      ...
      <button onClick={()=>go("registrar_mascota")} className="mp-registro mt-4 w-full rounded-3xl p-5 pb-3 text-left text-white relative overflow-hidden shadow-lg" style={{background:`linear-gradient(135deg, #0E7C6B, #0A5C50)`,boxShadow:"0 8px 24px rgba(14,124,107,.35)"}}>
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center shrink-0" style={{background:"rgba(255,255,255,.2)"}}><QrCode size={32}/></div>
          <div className="flex-1">
            <div className="text-[10px] font-extrabold tracking-wide opacity-90 mb-0.5">⭐ LO MÁS IMPORTANTE</div>
            <div className="font-extrabold text-lg leading-tight">Registrá tu mascota 🐾</div>
            <div className="text-[12px] opacity-90 mt-0.5 leading-snug">Creá su ficha con QR para el collar. Si se pierde, quien la encuentre te avisa al instante.</div>
          </div>
          <ChevronRight size={22} className="opacity-90 shrink-0"/>
        </div>
        {/* Patitas caminando (franja visible) */}
        <div className="flex items-center justify-center gap-4 mt-2.5 pt-2" style={{borderTop:"1px solid rgba(255,255,255,.15)"}}>
          <span className="mp-huella text-lg" style={{transform:"rotate(20deg)"}}>🐾</span>
          <span className="mp-huella text-lg" style={{transform:"rotate(-12deg) translateY(4px)"}}>🐾</span>
          <span className="mp-huella text-lg" style={{transform:"rotate(20deg)"}}>🐾</span>
          <span className="mp-huella text-lg" style={{transform:"rotate(-12deg) translateY(4px)"}}>🐾</span>
          <span className="mp-huella text-lg" style={{transform:"rotate(20deg)"}}>🐾</span>
          <span className="mp-huella text-lg" style={{transform:"rotate(-12deg) translateY(4px)"}}>🐾</span>
  ...
  return (
    <div className="px-4 pb-6">
      ...
      <div className="space-y-3">
        ...
        <Field label="WhatsApp de contacto *"><input inputMode="tel" value={f.phone} onChange={e=>set("phone",e.target.value)} className="inp" placeholder="Ej: 3764 123456"/>{f.phone&&!telValido(f.phone)&&<div className="text-[11px] mt-1 font-semibold" style={{color:C.lost}}>Escribí el número con la característica, sin 0 ni 15. Ej: 3764 123456</div>}</Field>
...
function DetailView({ post, all, go, setStatus, flash, onReport, user, onDelete }){
  ...
  const wa=()=>{const est=post.type==="lost"?"🔴 PERDIDA":post.type==="found"?"🟢 ENCONTRADA":"🟡 VISTA";const esp=post.species&&post.species!=="otro"?post.species:"mascota";const quien=post.petName?`${post.petName} (${esp})`:esp.charAt(0).toUpperCase()+esp.slice(1);const msg=`Hola 👋 Te escribo por tu publicación en Mascotas Perdidas Misiones:\n${est}: ${quien} en ${ubicTxt(post)}\n${linkPost(post)}\n\nCreo que tengo información que te puede servir.`;window.open(`https://wa.me/${waNum(post.whatsapp)}?text=${encodeURIComponent(msg)}`,"_blank");};
  return (
    <div>
...
function ShareView({ post, go, flash }){
  ...
  const share=(net)=>{ if(net==="wa")window.open(`https://wa.me/?text=${encodeURIComponent(text)}`,"_blank"); else if(net==="fb")window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(link)}&quote=${encodeURIComponent(text)}`,"_blank"); else if(net==="native"){if(navigator.share)navigator.share({title:"Mascotas Perdidas Misiones",text,url:link}).catch(()=>{});else{navigator.clipboard&&navigator.clipboard.writeText(text);flash("Copiado.");}} else {navigator.clipboard&&navigator.clipboard.writeText(text);flash("Texto copiado.");} };
  const afiche=async(modo)=>{
    ...
    try{
      ...
      if(modo==="share" && navigator.canShare && navigator.canShare({files:[file]})){
        await navigator.share({files:[file],title:"Mascotas Perdidas Misiones",text}); 
      } else {
...
function drawQROnCanvas(canvas,text,size,dark,light){
  ...
  if(!m){ // si la librería no cargó, dejar el canvas visible con aviso
    canvas.width=size; canvas.height=size; const ctx=canvas.getContext("2d");
    ctx.fillStyle="#FFFFFF"; ctx.fillRect(0,0,size,size); return canvas;
  }
  const count=m.size; const data=m.data;
...
function RegistrarMascotaView({ go, onSave, flash }){
  ...
  const canSave=f.petName&&f.photo&&telValido(f.phone);
  const guardar=async()=>{ f.phone=telLocal(f.phone); f.whatsapp=f.phone; if(!canSave){flash("Completá nombre, foto y un WhatsApp válido.");return;} setSaving(true); const m=await onSave(f); setSaving(false); if(m){ go("qr_mascota"); } };
  return (<div className="px-4 pb-6">
...
function QRMascotaView({ go, mascota, flash }){
  ...
  useEffect(()=>{ try{ if(canvasRef.current){ drawQROnCanvas(canvasRef.current,url,240,"#000000","#FFFFFF"); setListo(true); setQrError(false); } }catch(e){ setQrError(true); } },[url]);
  const descargarPoster=async()=>{
    const W=1080,H=1500; const cv=document.createElement("canvas"); cv.width=W;cv.height=H; const x=cv.getContext("2d");
    x.fillStyle="#FFFFFF";x.fillRect(0,0,W,H); x.strokeStyle="#0E7C6B";x.lineWidth=10;x.strokeRect(24,24,W-48,H-48);
    x.fillStyle="#0E7C6B";x.fillRect(24,24,W-48,150); x.fillStyle="#FFFFFF";x.textAlign="center";x.font="800 68px sans-serif";x.fillText("¡AYUDAME A VOLVER!",W/2,118);
    // foto
    await new Promise((res)=>{ if(mascota.photo){const img=new Image();img.crossOrigin="anonymous";img.onload=()=>{const r=Math.max(400/img.width,400/img.height),iw=img.width*r,ih=img.height*r;x.save();x.beginPath();x.arc(W/2,390,200,0,7);x.clip();x.drawImage(img,W/2-iw/2,390-ih/2,iw,ih);x.restore();res();};img.onerror=res;img.src=mascota.photo;}else res(); });
    x.fillStyle="#12211C";x.textAlign="center";x.font="800 70px sans-serif";x.fillText((mascota.pet_name||mascota.petName||"").toUpperCase(),W/2,660);
    x.fillStyle="#5F726B";x.font="400 32px sans-serif";x.fillText("Escaneá el código para avisar dónde estoy",W/2,708);
    // QR grande y NEGRO (máxima legibilidad)
    const qSize=440; const qcv=document.createElement("canvas"); drawQROnCanvas(qcv,url,qSize,"#000000","#FFFFFF");
    x.drawImage(qcv,W/2-qSize/2,750,qSize,qSize);
    // franja verde
