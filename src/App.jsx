...
const KEYS = { posts:"mp:posts:v4", reports:"mp:reports:v4", session:"mp:session:v4", lugares:"mp:lugares:v1", mensajes:"mp:mensajes:v1" };
  ...
  return (
    <div style={{background:C.bg,color:C.ink,fontFamily:"'Plus Jakarta Sans', system-ui, -apple-system, Segoe UI, Roboto, sans-serif"}} className="min-h-screen w-full">
      ...
      <div className="mx-auto max-w-[480px] relative pb-24" style={{background:C.bg}}>
        ...
        {view==="admin"   && (isAdmin(user) ? <AdminView posts={posts} reports={reports} approve={approve} removePost={removePost} go={go} clearReport={clearReport} conn={conn} mensajes={mensajes} borrarMensaje={borrarMensaje} lugares={lugares} guardarLugar={guardarLugar} borrarLugar={borrarLugar} /> : <AdminLocked go={go} />)}
...
async function generarAfiche(post){
  ...
  const drawResto=()=>{
    ...
    x.fillText((post.petName||post.species||"").toUpperCase(),W/2,boxY+boxH+90);
    const rows=[["Especie",(post.species||"")+(post.sex?" · "+post.sex:"")],["Color",post.color||"-"],["Zona",(post.barrio||"")+", Posadas"],["Fecha",post.date||"-"]];
    if(post.features)rows.push(["Señas",post.features]);
    let ry=boxY+boxH+150;
    ...
    x.fillText("Si la viste, comunicate:",W/2,cy+45);
    x.font="800 44px sans-serif"; x.fillText("WhatsApp "+(post.whatsapp||post.phone||""),W/2,cy+90);
    // Marca
    x.fillStyle="#0E7C6B"; x.font="800 36px sans-serif"; x.fillText("🐾 MASCOTAS POSADAS",W/2,H-90);
...
function AdminView({ posts, reports, approve, removePost, go, clearReport, conn, mensajes=[], borrarMensaje, lugares=[], guardarLugar, borrarLugar }){
  ...
  return (
    <div className="px-4">
      ...
      <div className="flex gap-2 mb-3 overflow-x-auto mp-scroll -mx-4 px-4">{[["stats","Estadísticas"],["mod","Moderación"],["reports",`Reportes${reports.length?` (${reports.length})`:""}`],["mensajes",`Mensajes${mensajes.length?` (${mensajes.length})`:""}`],["lugares","Lugares"],["estado","Estado"]].map(([k,l])=><button key={k} onClick={()=>setTab(k)} className="shrink-0 px-3 py-2 rounded-xl text-xs font-bold" style={{background:tab===k?C.ink:C.surface,color:tab===k?"#fff":C.muted,border:`1px solid ${tab===k?C.ink:C.line}`}}>{l}</button>)}</div>
