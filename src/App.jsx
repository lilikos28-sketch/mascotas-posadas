...
export default function App(){
  ...
  const enviarMensaje=async(m)=>{
    ...
    if(conn==="cloud"){ try{ const c=await getClient(); const {data,error}=await c.from("mensajes").insert(rec).select().single(); if(error)throw error; setMensajes(prev=>[data,...prev]); return true; }catch{ flash("No se pudo enviar. Reintentá."); return false; } }
  ...
  const registrarMascota=async(m)=>{
    ...
    if(conn==="cloud"){ try{ const c=await getClient(); let photo=rec.photo; if(photo&&photo.startsWith("data:"))photo=await uploadPhoto(c,photo); const {data,error}=await c.from("mascotas").insert({...rec,photo}).select().single(); if(error)throw error; setMisMascotas(prev=>[data,...prev]); return data; }catch(e){ flash("No se pudo registrar la mascota."); return null; } }
  ...
  const reportarAvistamiento=async(mascota,a)=>{
    ...
    if(conn==="cloud"){ try{ const c=await getClient(); const {data,error}=await c.from("avistamientos").insert(rec).select().single(); if(error)throw error; setAvistamientos(prev=>[data,...prev]); return true; }catch{ flash("No se pudo enviar el aviso."); return false; } }
  ...
  return (
    <div style={{background:C.bg,color:C.ink,fontFamily:"'Plus Jakarta Sans', system-ui, -apple-system, Segoe UI, Roboto, sans-serif"}} className="min-h-screen w-full">
      ...
      <div className="mx-auto max-w-[480px] relative pb-24" style={{background:C.bg}}>
        ...
        {view==="registrar_mascota"&& (requireAuthToPublish ? <AuthGate go={go} /> : <RegistrarMascotaView go={go} onSave={registrarMascota} flash={flash} />)}
        ...
        {view==="qr_mascota"&& qrMascota && <QRMascotaView go={go} mascota={qrMascota} flash={flash} />}
        {view==="mascota_publica"&& qrMascota && <MascotaPublicaView go={go} mascota={qrMascota} onReport={reportarAvistamiento} flash={flash} />}
...
function MapView({ posts, go }){
  ...
  return (
    <div className="px-4">
      <Title back={()=>go("home")} title="Mapa de Misiones" tag="PARCIAL"/>
...
/* ---- Generador de QR (carga librería por CDN, igual que Supabase) ---- */
let _qrlib=null;
async function ensureQR(){
  if(_qrlib)return _qrlib;
  if(window.QRCode){_qrlib=window.QRCode;return _qrlib;}
  await new Promise((res)=>{const s=document.createElement("script");s.src="https://cdn.jsdelivr.net/npm/qrcode@1.5.3/build/qrcode.min.js";s.onload=res;s.onerror=res;document.body.appendChild(s);setTimeout(res,7000);});
  _qrlib=window.QRCode||null; return _qrlib;
}
...
function RegistrarMascotaView({ go, onSave, flash }){
  ...
  const onPhoto=async(e)=>{ const file=e.target.files&&e.target.files[0]; if(!file)return; const data=await compressImage(file); set("photo",data); };
...
function QRMascotaView({ go, mascota, flash }){
  ...
  useEffect(()=>{ (async()=>{ const QR=await ensureQR(); if(QR&&canvasRef.current){ try{ await QR.toCanvas(canvasRef.current,url,{width:240,margin:1,color:{dark:"#0E7C6B",light:"#FFFFFF"}}); setListo(true); }catch{} } })(); },[url]);
  const descargarPoster=async()=>{
    const QR=await ensureQR(); if(!QR){flash("No se pudo generar el QR.");return;}
    ...
    // QR grande
    const qcv=document.createElement("canvas"); await QR.toCanvas(qcv,url,{width:360,margin:1,color:{dark:"#0E7C6B",light:"#FFFFFF"}});
